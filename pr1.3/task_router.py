...
from models import Task, TaskCategory, TaskDefault, TaskTimeLogDefault, TaskUpdateDefault, TaskTimeLog, Priority, User, Category
...
async def create_task(task_data: TaskCreate, session: AsyncSession = Depends(get_session)) -> TaskResponse:
...
async def get_all_tasks(session: AsyncSession = Depends(get_session)) -> TaskListResponse:
    result = await session.execute(
        select(Task)
        .options(selectinload(Task.user), selectinload(Task.categories), selectinload(Task.time_logs))
    )
...
async def get_task(task_id: int, session: AsyncSession = Depends(get_session)) -> TaskResponse:
    result = await session.execute(
        select(Task)
        .options(selectinload(Task.user), selectinload(Task.categories), selectinload(Task.time_logs))
...
@router.put("/{task_id}")
async def update_task(task_id: int, task_data: TaskCreate, session: AsyncSession = Depends(get_session)) -> MessageResponse:
...
async def delete_task(task_id: int, session: AsyncSession = Depends(get_session)) -> MessageResponse:
    result = await session.execute(select(Task).where(Task.id == task_id))
    task = result.scalars().first()
...
async def add_time_log(task_id: int, time_log_data: TaskTimeLogDefault, session: AsyncSession = Depends(get_session)) -> TaskTimeLogResponse:
...
async def delete_time_log(task_id: int, time_log_id: int, session: AsyncSession = Depends(get_session)) -> MessageResponse:
    result = await session.execute(select(Task).where(Task.id == task_id))
    task = result.scalars().first()
